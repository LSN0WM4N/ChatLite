import { useRef, useState } from "react";
import { useDispatch } from "react-redux";

import { removeLoading, setError, setLoading } from "../../features/ui/uiSlice";
import api from "../../api/axios";

import { ProfilePhoto } from "../profile/ProfilePhoto";

export const ChangePhoto = ({
    endpoint,
    body,
    type,
    canEdit = false,
    ...props
}) => {
    const dispatch = useDispatch();
    const [file, setFile] = useState(null);
    const inputRef = useRef(null);

    const handleFileChange = async (e) => {
        e.preventDefault();
        const upFile = e.target.files[0];
        if (!upFile)
            return;

        const url = URL.createObjectURL(upFile);
        setFile(url);
        dispatch(setLoading());

        const formData = new FormData();
        formData.append("photo", upFile);
        formData.append("data", JSON.stringify(body));

        try {
            await api.post(endpoint, formData, {
                headers: {
                    "Content-Type": "multipart/form-data",
                },
            });

        } catch (error) {
            console.error("Error while uploading:", error);
            dispatch(setError(error));
        } finally {
            dispatch(removeLoading());
        }

    };


    return (
        <div className="relative sm:w-48 flex flex-shrink-0 justify-center" {...props}>
            <ProfilePhoto
                ID={body.userID || body.channelID}
                size={96}
                preview={file}
                type={type}
            />

            {canEdit && (
                <>
                    <button
                        type="button"
                        onClick={() => inputRef.current.click()}
                        className="absolute bottom-2 right-2 bg-white rounded-full p-2 shadow-md hover:bg-gray-200"
                    >
                        <svg
                            className="w-5 h-5 text-gray-700"
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 35.324 35.324"
                            fill="currentColor"
                        >
                            <path d="M32.752,9.018L8.149,33.62c-0.241,0.242-0.545,0.41-0.878,0.486l-5.034,1.17c-0.613,0.141-1.258-0.043-1.703-0.488
                                c-0.446-0.444-0.629-1.09-0.487-1.703l1.166-5.03c0.078-0.334,0.246-0.64,0.488-0.881L26.306,2.572l2.578,2.579L4.71,29.321
                                c-0.356,0.355-0.356,0.937,0,1.291c0.356,0.357,0.934,0.355,1.291,0L30.176,6.439L32.752,9.018z M28.343,0.535l-0.989,0.987
                                l6.448,6.447L34.79,6.98c0.713-0.713,0.712-1.868,0-2.582l-3.865-3.864C30.211-0.177,29.055-0.177,28.343,0.535z"/>
                        </svg>
                    </button>

                    <input
                        type="file"
                        accept="image/*"
                        ref={inputRef}
                        onChange={handleFileChange}
                        className="hidden"
                    />
                </>
            )}
        </div>
    );
};